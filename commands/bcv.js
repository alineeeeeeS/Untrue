...
export async function bcvCommand(sock, m, args) {
    try {
        ...
        await sock.sendMessage(jid, {
            text: '?? *Consultando precios del BCV (Dólar y Euro)...*'
        }, { quoted: m });
...
async function getBCVFromHistorial(fechaInput) {
    try {
        ...
        if (historial[fechacorta]) {
            ...
            console.log(`?? Precios históricos encontrados: USD=${data.usdPrice}, EUR=${data.eurPrice}`);
            return {
...
async function guardarEnHistorial(fechacorta, usdPrice, eurPrice) {
    try {
        ...
        // Convertir el registro existente a una cadena JSON para comparación profunda
